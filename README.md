...
Version 1.3.6 fixes the Utilities modpack selector display text, updates the sidebar brand label to Ready Or Not, and uses the local Ready or Not executable icon for the app window when available. Version 1.3.5 polishes the Utilities and Settings UI by removing the white disabled dropdown chrome, centering the Settings page header, and showing clean colour scheme names in the selector.
